...
function Profile() {
  ...
  const fetchProfilePicture = useCallback(async () => {
    ...
    if (storedProfileImageUrl) {
      ...
      console.log(
        "Profile picture URL loaded from session storage:",
    ...
    try {
      ...
      if (response && response.profilePictureUrl) {
        ...
        console.log(
          "Profile picture URL fetched and stored in session storage:",
          cacheBustedUrl
        );
      } else {
  ...
  useEffect(() => {
    ...
    const fetchWorkouts = async () => {
      try {
        ...
        const recent4Workouts = await getAllWorkouts(user.username, {
          count: 4,
        });
        const last5WeeksWorkouts = await getAllWorkouts(user.username, {
          days: 35,
        });
    ...
    fetchWorkouts();
  }, [user.username, fetchProfilePicture]);
