...
function Workouts() {
  ...
  const fetchWorkouts = async () => {
    try {
      ...
      if (data && Array.isArray(data)) {
        ...
        const sortedWorkouts = formattedWorkouts.sort(
    ...
    if (deleteType === "workout") {
      ...
      try {
        ...
        const updatedSplits = userSplits.filter(
          (split) => split.splitId !== itemToDelete
        );
    ...
    try {
      ...
      if (action === "save") {
        ...
        const itemIndex = updatedItems.findIndex(
          (i) => i[`${itemType}Id`] === item[`${itemType}Id`]
        );
        if (itemIndex !== -1) {
      ...
      console.log(
        `${
          messageAction.charAt(0).toUpperCase() + messageAction.slice(1)
        } ${itemType}: `,
        itemWithDate
      );
  ...
  return (
    <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ...
            {deleteType === "workout" ? "workout" : "split"}? This action cannot
            be undone.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={isEditDialogOpen} onClose={handleCloseEditDialog}>
        ...
        <DialogContent>
          ...
          {userSplits.length > 0 ? (
            userSplits.map((split, index) => (
              ...
              >
                <TextField
                  value={
                    tempSplitName[index] !== undefined
                      ? tempSplitName[index]
                      : split.name
                  }
                  onChange={(e) => {
                    const updatedTempSplitName = {
                      ...tempSplitName,
                      [index]: e.target.value,
                    }; // Update temp name on change
                  ...
                  onBlur={() => {
                    ...
                    if (
                      tempSplitName[index] !== undefined &&
                      tempSplitName[index] !== split.name
                    ) {
                ...
                <IconButton
                  color="error"
                  onClick={() => handleDeleteSplit(split.splitId)}
                >
                  <DeleteIcon />
                </IconButton>
              </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", mt: 4 }}>
            {console.log("Current number of splits:", userSplits.length)}{" "}
            {/* Log the length of userSplits */}
            <Button
              variant="outlined"
              startIcon={<AddIcon />}
              onClick={() => {
      ...
      <Dialog
        open={isCustomSplitDialogOpen}
        onClose={() => setIsCustomSplitDialogOpen(false)}
      >
          ...
          <Button
            ...
            onClick={() => {
              if (!customSplitName.trim()) {
                // Check if the customSplitName is empty or only contains spaces
