  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        setErrorMessage(
          "Failed to submit your message. Please check your input and try again."
        ); // Set error message
      }
    } catch (error) {
  ...
  return (
    <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <Box sx={{ flexGrow: 1, textAlign: "center", mt: 4 }}>
        ...
        <Typography
          variant="body1"
          sx={{ mb: 4, maxWidth: "800px", margin: "0 auto" }}
        >
          Have any questions, feedback, or just want to get in touch? Fill out
