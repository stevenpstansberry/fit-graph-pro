...
export const getTitle = (
  ...
  const componentName =
    component === "heatmap"
      ? "Heatmap"
      : component === "strengthChart"
      ? "Strength Chart"
      : "View Workouts";

  if (component === "strengthChart") {
    if (timeframe === "currentMonth") {
      return ` ${componentName} for: ${toTitleCase(
        selectedExercise
      ...
      return ` Year-to-Date ${componentName} for ${toTitleCase(
        selectedExercise
      )} from Jan 1 ${selectedYear} through ${todayDate}`;
    } else if (timeframe === "allTime") {
      return ` All-Time ${componentName} for: ${toTitleCase(selectedExercise)}`;
    } else {
      return `Displaying Workout History for: ${toTitleCase(
        selectedExercise
      )}}`;
    }
  } else {
