...
const FuturePrediction = ({ workoutHistory = [] }) => {
  ...
  const handlePredict = async () => {
    if (!selectedExercise || !goalWeight || !workoutHistory.length) {
      console.error(
        "Please select an exercise, set a goal weight, and ensure workout history is available."
      );
      return;
    }
    ...
    const exerciseCount = workoutHistory.flatMap((workout) =>
      workout.exercises.filter(
        (exercise) => exercise.label === selectedExercise
      )
    ).length;

    if (exerciseCount <= 1) {
      setSnackbarMessage(
        "You need at least two workout entries of the selected exercise to predict future performance."
      );
    ...
    try {
      ...
      if (predictedDate < today) {
        setPredictionResult(
          `Predicted Date: ${response.predictedDate} (Note: This date is in the past. This may indicate a small or strange data distribution.)`
        );
      } else {
        setPredictionResult(
          `Predicted Achievement Date: ${response.predictedDate}`
        );
      }
    } catch (error) {
      console.error("Error predicting future performance:", error);
      setPredictionResult(
        "An error occurred while predicting future performance."
      );
    }
  };
  ...
  const handleEstimate1RM = () => {
    if (!selectedExercise || !workoutHistory.length) {
      console.error(
        "Please select an exercise and ensure workout history is available."
      );
      return;
    }
    ...
    if (maxSet) {
      ...
      const estimated1RM = (maxSet.weight * (1 + 0.0333 * maxSet.reps)).toFixed(
        2
      );
      setOneRMResult(
        `Your estimated 1RM for ${selectedExercise} is ${estimated1RM} lbs based on your best set: ${maxSet.weight} lbs x ${maxSet.reps} reps.`
      );
    } else {
      setOneRMResult(
        `No data available for estimating 1RM for ${selectedExercise}.`
      );
    }
  };
      ...
      <Container sx={{ mt: 4, pb: 8, flexGrow: 1 }}>
        ...
        {tabIndex === 0 && (
          <Box sx={{ mt: 4 }}>
            ...
            <Typography variant="body1" sx={{ mb: 3 }}>
              Enter your goal weight and select an exercise to predict when you
              will reach your goal based on your workout history. For accurate
              predictions, it's suggested to have at least 50 sets logged per
              exercise. This typically corresponds to 3 to 6 months of regular
              entries, at about 2-3 workouts per week.
              <Tooltip
                title={
                  <Typography sx={{ color: "#e0f7fa" }}>
                    We employ advanced linear regression models to analyze your
                    historical workout data. By evaluating your rate of
                    progression, we estimate when you'll achieve your target
                    weight. This involves statistically modeling your
                    performance curve and calculating the optimal point of
                    future success!
                  </Typography>
                }
                arrow
            ...
            <Grid container spacing={3} sx={{ mt: 2 }} alignItems="center">
              ...
              <Grid item xs={12} md={4}>
                <Autocomplete
                  options={exerciseLabels.map((label) => toTitleCase(label))}
                  value={toTitleCase(selectedExercise)}
            ...
            >
              <Tooltip
                title={
                  !goalWeight
                    ? "Please enter a goal weight to enable prediction."
                    : ""
                }
                arrow
                placement="top"
        ...
        {tabIndex === 1 && (
          <Box sx={{ mt: 4 }}>
            ...
            <Typography variant="body1" sx={{ mb: 3 }}>
              Select an exercise to estimate your 1-rep max (1RM) based on your
              workout history. We will search for your set with the highest
              weight and, if there are multiple sets with the same weight, the
              set with the highest reps.
              <Tooltip
                title={
                  <Typography sx={{ color: "#e0f7fa" }}>
                    1RM estimation uses your heaviest set with the highest reps
                    to calculate your theoretical maximum weight for a single
                    repetition. This is often done using Epley's formula: 1RM =
                    Weight x (1 + 0.0333 x Reps).
                  </Typography>
                }
                arrow
                componentsProps={{
